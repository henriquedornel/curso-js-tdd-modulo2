<template>
  <div class="bg-white">
    ...
    <cart
      :products="/* $cart.getState().items */ products"
      :is-open="/* $cart.getState().open */ isCartOpen"
      @close="toggleCart"
    />
    <nuxt />
...
export default {
  ...
  computed: {
    isCartOpen() {
      // essa computed property é necessária para poder utilizar dentro do template (cartManager não é reconhecido diretamente no template, pois está fora de "export default", e só pode ser utilizada dentro do script)
  ...
  methods: {
    toggleCart() {
      // cartManager.getState().open ? cartManager.close() : cartManager.open();
