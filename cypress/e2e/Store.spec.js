...
context('Store', () => {
  ...
  beforeEach(() => {
    server = makeServer({ environment: 'test' }); // quando o environment é "test", o mirage não vai abastecer o banco de dados quando o server é criado
  });
  ...
  context('Store > Shopping Cart', () => {
    ...
    it('should toggle shopping cart visibility when button is clicked', () => {
      // const toggleButton = gid('toggle-button'); // assim não funciona, pois o Cypress trabalha de forma assíncrona, e cy.get vai retornar uma promise
      gid('toggle-button').as('toggleButton');
      g('@toggleButton').click();
      gid('shopping-cart').should('not.have.class', 'hidden');

      g('@toggleButton').click({ force: true }); // para não dar o erro "failed because the center of this element is hidden from view"
      gid('shopping-cart').should('have.class', 'hidden');
    });
    ...
    it('should add 3 products to the cart', () => {
      cy.addToCart({ indexes: [1, 3, 5] }); // segundo, quarto e sexto produto

      gid('cart-item').should('have.length', 3);
    });

    it('should add 1 product to the cart', () => {
      cy.addToCart({ index: 6 }); // sétimo produto
